...
const Graficos = () => {
  ...
  // Query para ventas diarias (minoristas)
  const { data: ventasDiarias = [], isLoading: loadingVentas } = useQuery({
    queryKey: ["ventas-diarias-minoristas"],
    queryFn: async () => {
      const res = await API.get("/graficos/ventas-diarias");
  ...
  // Query para top productos (minoristas)
  const { data: topProductos = [], isLoading: loadingProductos } = useQuery({
    queryKey: ["top-productos-minoristas"],
    queryFn: async () => {
      const res = await API.get("/graficos/top-productos");
  ...
  // Query para ventas por empleados (minoristas)
  const { data: ventasEmpleados = [], isLoading: loadingEmpleados } = useQuery({
    queryKey: ["ventas-empleados-minoristas"],
    queryFn: async () => {
      const res = await API.get("/graficos/ventas-empleados");
  ...
  // Query para métricas (minoristas)
  const { data: metricas, isLoading: loadingMetricas } = useQuery({
    queryKey: ["metricas-minoristas"],
    queryFn: async () => {
      const res = await API.get("/graficos/metricas");
      return res.data;
    },
  });
  ...
  if (isLoading) {
    return (
      <div className="estado-carga">
        Cargando datos del dashboard minoristas...
      </div>
    );
  }
  ...
  return (
    <main className="pagina-container pagina-graficos">
      <header className="pagina-header header-graficos">
        <h1>Graficos Ventas Minoristas</h1>
      </header>
        ...
        <section className="metricas-graficos">
          ...
          <article className="card-metrica stock">
            ...
            <div className="valor-metrica alerta">
              {metricas.inventario.productosBAjoStock}
            </div>
            <div className="detalle-metrica">
              <span>Productos con ≤10 unidades</span>
            </div>
          </article>
        </section>
      )}
      ...
      <section className="card-grafico">
        <div className="header-card-grafico">
          <h2>Ventas Diarias Minoristas - Últimos 30 días</h2>
        </div>
      ...
      <section className="grid-graficos-doble">
        ...
        <article className="card-grafico">
          <div className="header-card-grafico">
            <h2>Top 10 Productos Minoristas</h2>
          </div>
          <div className="contenedor-grafico">
            <ResponsiveContainer width="100%" height={300}>
        ...
        <article className="card-grafico">
          <div className="header-card-grafico">
            <h2>Ventas por Empleado (Minoristas)</h2>
          </div>
          <div className="contenedor-grafico">
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
      ...
      <section className="card-tabla">
        <div className="header-tabla">
          <h2>Detalle de Ventas por Empleado (Minoristas)</h2>
        </div>
